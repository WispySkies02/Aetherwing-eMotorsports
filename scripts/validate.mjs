...
const d2Event = scheduleEvents.find((event) => event.league === 'uarl-d2' && event.date === '2026-09-18');
assert(d2Event?.time === '6:45 PM ET' && /Daytona 250/i.test(d2Event?.title || ''), 'September 18 UARL D2 Daytona opener is Friday at 6:45 PM ET');
...
assert(schedulePage.includes('data-league-share') && schedulePage.includes('/schedule/share/${encodeURIComponent(selected.key)}/?v=52'), 'Schedule can share the currently selected league');
...
assert(leagueShareRoute.includes('/images/social/leagues/${league}-v52.jpg') && leagueShareRoute.includes('&#8203;') && !leagueShareRoute.includes('og:description'), 'League embeds use image-only v52 next-race cards with race-type badges');
for (const league of ['nrrs','kmart','sunoco','uarl-all','uarl-d1','uarl-d2','open','iracing']) { assert(existsSync(resolve(root, 'public/images/social/leagues', `${league}-v52.jpg`)), `League share image exists: ${league}`); }
